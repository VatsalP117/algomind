...
chrome.runtime.onMessage.addListener((message, _, sendResponse) => {
    ...
    (async () => {
        switch (message.type) {
            ...
            case 'algomind:pair': {
                const session = await pairExtension(message.code, message.installationName);
...
async function pairExtension(code, installationName) {
    const settings = await getSettings();
...
async function saveProblem(pageContext) {
    ...
    const doSave = async (token) => {
        ...
        if (!response.ok) {
            throw new Error(readErrorMessage(payload, 'Failed to save problem to Algomind'));
        }
        return payload;
    };
...
async function refreshSession(currentSession) {
    ...
    refreshInFlight = (async () => {
        const settings = await getSettings();
        const response = await fetch(`${settings.apiBaseUrl}/extension/auth/refresh`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            await clearSession();
            throw new Error(readErrorMessage(payload, 'Failed to refresh extension session'));
        }
...
function mapPairResponseToSession(payload) {
    return {
