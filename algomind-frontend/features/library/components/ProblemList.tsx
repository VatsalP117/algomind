    ...
    return (
        <div className="space-y-4">
            {problems.map((problem) => (
                <Link href={`/dashboard/library/problem/${problem.id}`} key={problem.id} className="block group">
                        ...
                        <CardContent className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 gap-4">
                            <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                    <h3 className="font-semibold text-base sm:text-lg group-hover:text-primary transition-colors">{problem.title}</h3>
